...
void loop_task_proc(void *arg)
{
	...
	while(1)
	{
		...
		else
		{
			...
			compensated_pose_vector[0] = compensated_pose_vector[0] + pid_compensation[0]; //+ ros_state->get_rl_action()[0];
			compensated_pose_vector[1] = compensated_pose_vector[1] + pid_compensation[1]; //+ ros_state->get_rl_action()[1];
			compensated_pose_vector[2] = compensated_pose_vector[2] + pid_compensation[2]; //+ ros_state->get_rl_action()[2];
		...
		{
			test_q_[num] = solutions[1].toStdVector()[num];
		}
		...
		for(int num = 0; num <6 ; num ++)
		{
			if(fabs((test_q_[num] - current_q[num])/control_time) > 270*DEGREE2RADIAN)
			{
				cout << "::" << num << "::" << fabs((test_q_[num] - current_q[num])/control_time) << endl;
		...
		if(!joint_vel_limits)
		{
			...
			else
			{
				...
				ros_state->send_gazebo_command(test_q_);
				//ros_state->send_gazebo_b_command(test_q_);

			}
			for(int num = 0; num <6 ; num ++)
			{
				current_q[num] = test_q_[num];
			}
		}
		...
		previous_t = (rt_timer_read() - tstart)/1000000.0;
		cout << COLOR_GREEN_BOLD << "Check task's completion A : " << task_completed << "  Send time : "<< previous_t << COLOR_RESET << endl;
...
//	}
	...
	while(1)
	{
		...
		cout << COLOR_YELLOW_BOLD << " B " << task_completed << COLOR_RESET << endl;
...
//		previous_task_command = ros_state->get_task_command();
		...
		previous_t = (rt_timer_read() - tstart)/1000000.0;
		cout << COLOR_GREEN_BOLD << "Check task's completion B : " << task_completed << "  Send time : "<< previous_t << COLOR_RESET << endl;
		rt_task_wait_period(NULL);
	}
}
void initialize()
...
int main (int argc, char **argv)
{
	...
	CPU_SET(3, &mask_a);    // select CPU 3
	CPU_SET(1, &mask_b);    // select CPU 1

	//real time task
	char str[35];
	sprintf(str, "Belt Task A Start");
	rt_task_create(&loop_task, str, 0, 49, 0);//Create the real time task
	rt_task_set_affinity(&loop_task, &mask_a);
	usleep(3000000);
	rt_task_start(&loop_task, &loop_task_proc, 0);//Since task starts in suspended mode, start task

	sprintf(str, "Belt Task B Start");
	rt_task_create(&loop_task_b, str, 0, 50, 0);//Create the real time task
	rt_task_set_affinity(&loop_task_b, &mask_b);
	usleep(3000000);
