...
void TaskRobot::move_to_init_pose()
{
	if(!gazebo_check_)
	{
		std::cout << COLOR_RED_BOLD << robot_name_ <<": will move 2 seconds later" << COLOR_RESET << std::endl;

		usleep(2000000);

		//rtde_control_->moveJ(current_q_,0.1,0.1); // move to initial pose
...
{
	if(!command.compare("auto") && !robot_name_.compare("robot_B"))
		return false;

	if(!command.compare(""))
		return false;

	if(!command.compare("auto") && !robot_name_.compare("robot_A"))
	{
		static int previous_phases_ = 0;
		robot_task_->auto_task_motion(contact_check_);

		if(robot_task_->get_phases_() == 1 && previous_phases_ == 2)
		{
			std::cout << "Zero ft sensor" << std::endl;
			tool_estimation_->set_sensor_offset_value(raw_ft_data_);
		}
		previous_phases_ = robot_task_->get_phases_();
	}
	else
	{
		if(!command.compare("set_point"))
		{
			...
			//set_point_vector_ = ros_state->get_set_point();
			//ur10e_task_->set_point(set_point_vector[0], set_point_vector[1], set_point_vector[2], set_point_vector[3], set_point_vector[4], set_point_vector[5]); // base frame
